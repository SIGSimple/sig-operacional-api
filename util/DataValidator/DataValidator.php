...
class DataValidator {
    ...
    protected function set_error($error){
        $error = empty($this->msg_error) ? $error : $this->msg_error ; 
    ...
    /**
     * Verify if the current data is not CPF 
     * @access public
     * @return Data_Validator The self instance
     */
    public function is_cep() {
        $cep = trim($this->_data['value']);
        $avaliaCep = ereg("^[0-9]{8}$", $this->_data['value']);
        
        if(!$avaliaCep) {            
           $this->set_error(sprintf($this->_messages['is_cep'], $this->_data['name']));
        }
        return $this;
    }
    ...
    public function is_ip(){
        if(filter_var($this->_data['value'], FILTER_VALIDATE_IP) === FALSE) {
            $this->set_error(sprintf($this->_messages['is_ip'], $this->_data['name']));
        }
